            ...
            if self.prune:
                for n in descendants:
                    if self.debug_pruning:
                        write_hsdag(self.tree, 'hs-dag-step-%d.pdf' % self.i, highlight={'pink': [n]},
                                    title="deleting node %s" % (n))
                        self.i += 1
            ...
            if sigma < s_prime:
                for n_prime in list(tree.set_cache.get(s_prime, [])):
                    ...
                    if n_prime.tree is not None:
                        ...
                        for n in list(tree.neighbors(n_prime)):
                            if tree.edge_label((n_prime, n))[0] not in map(first,
                                                                           n_prime.set):  # <-- custom check for fault modes
                                self.cut_edge((n_prime, n))
